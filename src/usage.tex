...
\subitem - DEFB,DEFW,DEFI or DEFR directives with more than one parameter use the address of the first byte, when using \$ %That's why AS80 does not generate the same thing when using two DEFB instead of one with two parameters when using \$ reference in it.
...
Ces options sont utlisées,mais peuvent parfois etre utiles pour le débug. 
\begin{itemize}
